...
1. PUT 2/3
    ...
    Установлено, что предел пропускной способности - на уровне 6300 rps. **Улучшение на 26% (1300 rps)**

        Средняя задержка: 0.61 мс -> на 26% меньше
        90-%ная квантиль: 0.95 мс -> на 25% меньше
        99-%ная квантиль: 4.08 мс -> на 20% меньше

    Постоянная нагрузка 5000 rps -
    https://overload.yandex.net/148606

        Средняя задержка: 0.85 мс -> на 1% меньше
        90-%ная квантиль: 1.33 мс -> на 4% меньше
        99-%ная квантиль: 6.30 мс -> на 18% больше
...
2. PUT 3/3
    ...
    Предел пропускной способности - на уровне 5000 rps. **Без изменений**

        Средняя задержка: 0.80 мс -> на 2% меньше
        90-%ная квантиль: 1.38 мс -> на 4% меньше
        99-%ная квантиль: 4.75 мс -> на 4% меньше

    Постоянная нагрузка 5000 rps -
    https://overload.yandex.net/148622

        Средняя задержка: 0.84 мс -> на 2% меньше
        90-%ная квантиль: 1.38 мс -> на 3% больше
        99-%ная квантиль: 4.99 мс -> на 2% больше
...
3. GET 2/3
    ...
    Установлено, что предел пропускной способности - на уровне 12000 rps. **Улучшение на 60% (4500 rps)**

        Средняя задержка: 0.31 мс -> на 42% меньше
        90-%ная квантиль: 0.43 мс -> на 43% меньше
        99-%ная квантиль: 1.85 мс -> на 47% меньше

    Постоянная нагрузка 7500 rps -
    https://overload.yandex.net/148644

        Средняя задержка: 0.31 мс -> на 48% меньше
        90-%ная квантиль: 0.45 мс -> на 54% меньше
        99-%ная квантиль: 1.89 мс -> на 44% меньше
    ...
    Таким образом, приложение успешно справляется с возросшей пропускной способностью с незначительным увеличением задержек по сравнению с 7500 rps.
...
4. GET 3/3
    ...
    Установлено, что предел пропускной способности - на уровне 11000 rps. **Улучшение на 47% (3500 rps)**

        Средняя задержка: 0.35 мс -> на 35% меньше
        90-%ная квантиль: 0.50 мс -> на 43% меньше
        99-%ная квантиль: 2.36 мс -> на 31% меньше

    Постоянная нагрузка 7500 rps -
    https://overload.yandex.net/148658

        Средняя задержка: 0.33 мс -> на 45% меньше
        90-%ная квантиль: 0.47 мс -> на 52% меньше
        99-%ная квантиль: 2.11 мс -> на 35% меньше
    ...
    Как видно, увеличение нагрузки до предельной лишь незначительно увеличивает задержки.
...
5. PUT+GET 2/3
    ...
    Установлено, что предел пропускной способности - на уровне 7300 rps. **Улучшение на 22% (1300 rps)**

        Средняя задержка: 0.53 мс -> на 19% меньше
        90-%ная квантиль: 0.80 мс -> на 37% меньше
        99-%ная квантиль: 3.54 мс -> на 12% больше

    Постоянная нагрузка 6000 rps -
    https://overload.yandex.net/148669    

        Средняя задержка: 0.47 мс -> на 29% меньше
        90-%ная квантиль: 0.75 мс -> на 27% меньше
        99-%ная квантиль: 3.17 мс -> на 2% больше
...
6. PUT+GET 3/3
    ...
    Установлено, что предел пропускной способности - на уровне 7000 rps. **Улучшение на 17% (1000 rps)**

        Средняя задержка: 0.52 мс -> на 16% меньше
        90-%ная квантиль: 0.75 мс -> на 26% меньше
        99-%ная квантиль: 3.51 мс -> на 14% больше

    Постоянная нагрузка 6000 rps -
    https://overload.yandex.net/148672

        Средняя задержка: 0.56 мс -> на 18% меньше
        90-%ная квантиль: 0.83 мс -> на 25% меньше
        99-%ная квантиль: 3.65 мс -> на 13% меньше

### Результаты профилирования

Результаты профилирования с помощью async-profiler в том же эксперименте PUT+GET 3/3:

[![FlameGraph after optimizations](imgs/profile_after.svg)](imgs/profile_after.svg)

Можно заметить, что благодаря сделанным оптимизациям доля процессорного времени, потраченного на вызовы H2Dao.getValue(),
уменьшилась с 19.52% до 10.67%, то есть операция GET была ускорена почти в 2 раза (что согласуется с результатами выше).
Часть графика над ThreadPoolExecutor.runWorker() стала несколько шире: её доля возрасла с 12.83% до 20.41% - это следствие
добавления TaskScheduler'а и более активного использования классов из java.util.concurrent, благодаря которым удалось
